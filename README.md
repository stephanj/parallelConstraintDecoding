...
- **`java/`** — a from-scratch Java implementation of the same technique on
  ...
  of the three engines measured here.**
...
java -jar target/pcd-benchmark.jar                          # all presets, cold + warm rows
java -jar target/pcd-benchmark.jar -v --single-pass ../presets/fintech_fraud.json
```
...
```
presets/            four scenario JSONs (context + schema), shared by both engines
python/
...
java/
  ...
  src/main/java/pcd/
    NativeBenchmark.java   main: java -jar target/pcd-benchmark.jar [flags] [preset...]
    Preset.java            preset loading
    Prompts.java           same schema catalog text as python/core/schema.py
